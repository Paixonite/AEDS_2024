  ...
  if (!estaArrastando) {
    ...
    if (mouseButton == LEFT) {
      if(coresCelulas.containsKey(chave)){
        int antigaCor = coresCelulas.get(chave);
        int novaCor = color(0);
       
        if (antigaCor == color(255))
          novaCor = color(0);
        if (antigaCor == color(0,255,0))
          novaCor = color(255,0,0);
        if (antigaCor == color(0))
          novaCor = color(0,255,0);
        if (antigaCor == color(255,0,0))
          novaCor = color(0,0,255);
        if (antigaCor == color(0,0,255))
          novaCor = color(255,255,0);
       
        coresCelulas.put(chave, novaCor);
       
        if (antigaCor == color(255,255,0))
